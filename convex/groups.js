...
export const getGroupExpenses = query({
  ...
  handler: async (ctx, { groupId }) => {
    ...
    // pair‑wise ledger  debtor -> creditor -> amount i.e. ledger[A][B] = X means A owes B X
    const ledger = {};
    ids.forEach((a) => {
      ledger[a] = {};
      ids.forEach((b) => {
    ...
    for (const exp of expenses) {
      ...
      for (const split of exp.splits) {
        ...
        // Update the pair-wise ledger: increase what the debtor owes the creditor
    ...
    for (const s of settlements) {
      // settlement is from paidByUserId -> receivedByUserId
      totals[s.paidByUserId] += s.amount; 
      totals[s.receivedByUserId] -= s.amount;

      // Update the pair-wise ledger: reduce what the debtor owes the creditor
      ledger[s.paidByUserId][s.receivedByUserId] -= s.amount; // they paid back what they owed
    }
